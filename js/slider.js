...
class Slider {
	...
	constructor(container, opts = {}) {
		const d = { min: 0, max: 100, stp: 1, num: 1, col: "#4A90E2", fmt: v => v.toFixed(0), val: null, rng: 0 };
		opts = {...d, ...opts};
		...
		[this._min, this._max, this._step, this._range] = [opts.min, opts.max, opts.stp, opts.max - opts.min];
		[this._disp, this._colors, this._ranged] = [opts.fmt, [opts.col].flat(), opts.rng];
		[this._activeIdx, this._isMoving] = [-1, false];
		this._frameLoop = null;

		// Values initialization
		const values = opts.val ? [opts.val].flat() : this._defaultValues(opts.num);
	...
	_createElements() {
		const c = cls => Object.assign(document.createElement("div"), {className: cls});
		[this.rail, this.track, this.labelBox] = ["rail", "line", "lbls"].map(c);
		this.intervals = Array(Math.max(this._num, this._colors.length)).fill(0).map(() => c("range"));
		this.valueLabels = Array(this._num).fill(0).map(() => c("val"));
	...
	_setupEvents() {
		const move = e => this._activeIdx !== -1 && this._onMove(e),
			  up = e => this._activeIdx !== -1 && this._onUp(e);

		this.rail.addEventListener("pointerdown", e => this._onDown(e));
	...
	_defaultValues(n) {
		...
		return Array(n).fill(0).map((_, i) => this._roundToStep(this._min + step * (i + 1)));
	}

	get values() {
		const vals = this._vals.map(v => this._roundToStep(v));
		if(this._num === 1) return vals[0];
		if(this._colors.length >= this._num) return vals;
		...
		return Array(sorted.length - 1).fill(0)
			.map((_, i) => [sorted[i], sorted[i + 1]]);
	}

	set values(v) {
		const newVals = Array.isArray(v) ? (Array.isArray(v[0]) ? v.flat() : v) : [v];
		if(newVals.length !== this._num) throw new Error(this._num + " values expected");
	...
	_calculateLabelPositions(pos) {
		const rWidth = this.rail.offsetWidth, minGap = 4;
		const labels = pos.map((p, i) => ({
			origPos: p,
			origPx: (p / 100) * rWidth,
			idx: i,
			width: this.valueLabels[i].offsetWidth,
			halfWidth: this.valueLabels[i].offsetWidth / 2
		})).sort((a, b) => a.origPx - b.origPx);
		...
		const processEdge = (l, left) => {
			...
			labels.filter(o => o !== l && Math.abs(o.origPx - l.origPx) < l.width + minGap)
				.forEach((n, i) => {
		...
		labels.filter(l => l.origPx < l.halfWidth).forEach(l => processEdge(l, true));
		labels.filter(l => l.origPx > rWidth - l.halfWidth).forEach(l => processEdge(l, false));

		for(let iter = 0, hasOverlap = true; hasOverlap && iter < 50; iter++) {
			hasOverlap = false;
			for(let i = 0; i < labels.length - 1; i++) {
				const [c, n] = [labels[i], labels[i + 1]],
					  minSpace = (c.width + n.width) / 2 + minGap,
	...
	_updateUI() {
		const pos = this._vals.map(v => ((v - this._min) / this._range) * 100),
			  sorted = this._vals.map((v, i) => ({v, i})).sort((a, b) => b.v - a.v).map(x => x.i),
			  lift = new Set();

		if(this._activeIdx !== -1) {
			const aPos = pos[this._activeIdx];
			pos.forEach((p, i) => i !== this._activeIdx && Math.abs(p - aPos) < 4 && lift.add(i));
		}

		if(this._colors.length >= this._num) {
			pos.forEach((p, i) => this._updateLine(this.intervals[i], 0, p, this._colors[i], sorted.indexOf(i) + 1));
		} else {
			if(this._num % 2 === 0) {
				for(let i = 0; i < this._num; i += 2) {
					const [p1, p2] = [pos[i], pos[i + 1]], [l, r] = p1 < p2 ? [p1, p2] : [p2, p1];
					this._updateLine(this.intervals[i/2], l, r-l, this._colors[(i/2) % this._colors.length], i/2 + 1);
				}
			} else {
				const sVals = this._vals.map((v, i) => ({v, i})).sort((a, b) => a.v - b.v);
				for(let i = 0; i < sVals.length - 1; i++) {
					const [p1, p2] = [pos[sVals[i].i], pos[sVals[i + 1].i]];
					this._updateLine(this.intervals[i], Math.min(p1, p2), Math.abs(p2 - p1), 
						this._colors[i], i + 1);
				}
			}
		}
		...
		this._vals.forEach((val, i) => {
			let cIdx = this._colors.length >= this._num ? i : 
				this._num % 2 === 0 ? Math.floor(i/2) % this._colors.length :
				Math.min([...this._vals].sort((a,b) => a-b).indexOf(val), this._colors.length - 1);

			const label = this.valueLabels[i];
			label.textContent = this._disp(val);
			label.classList.toggle("lift", lift.has(i));
			this._updateLabel(label, adjPos[i], this._colors[cIdx], sorted.indexOf(i) + this._num + 1);
		});

		[this.minLabel.textContent, this.maxLabel.textContent] = [this._min, this._max].map(v => this._disp(v));
	}

	_updateLine(el, left, width, color, z) {
		Object.assign(el.style, {left: left + "%", width: width + "%", backgroundColor: color, zIndex: z});
	}
	...
	_updateLabel(el, left, color, z) {
		...
		if(pxPos < edge) tX = -((pxPos / edge) * 50);
		else if(pxPos > rW - edge) tX = -100 + ((rW - pxPos) / edge) * 50;
	...
	_onDown(e) {
		...
		this._activeIdx = this._vals.reduce((c, _, i) => 
			Math.abs(val - this._vals[i]) < Math.abs(val - this._vals[c]) ? i : c, 0);
	...
	_onMove(e) {
		...
		if(this._ranged && this._num % 2 === 0 && this._colors.length < this._num) {
			const pIdx = this._activeIdx % 2 === 0 ? this._activeIdx + 1 : this._activeIdx - 1,
			...
			if(isLower ? newVal > pVal : newVal < pVal) {
				...
				[this._activeIdx, this._startVal, this._startPos] = [pIdx, pVal, pos];
				return this._animate();
			}
			newVal = isLower ? Math.min(newVal, pVal - this._ranged) : 
	...
	_dispatch(type) {
		this._dispatcher.dispatchEvent(new CustomEvent(type, { detail: this.values }));
	}
