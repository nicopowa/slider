 ...
 * SLIDER STANDALONE RELEASE v0.2.7
 * 27/01/2025 17:48:23
 */
(function(){
...
class C{constructor(a,b={}){this.W=String(b.stp||"").includes(".")?String(b.stp).split(".")[1].length:0;b={min:0,max:100,stp:1,num:1,col:"#4A90E2",fmt:e=>e.toFixed(this.W),val:null,rng:0,...b};this.V=this.D=this.S=this.O=this.R=this.P=this.G=this.B=null;this.L=this.M=0;[this.o,this.T,this.N,this.C]=[b.min,b.max,b.stp,b.max-b.min];[this.I,this.m,this.K]=[b.fmt,[b.col].flat(),b.rng];[this.l,this.F]=[-1,!1];this.B=null;b=b.val?[b.val].flat():q(this,b.num);this.g=b.length;this.i=Array(this.g).fill(0);
this.u=Array(this.g).fill(0);this.j=a;this.j.classList.add("slider");t(this);u(this);this.values=b;this.J=new D}get values(){const a=this.i.map(e=>r(this,e));if(1===this.g)return a[0];if(this.m.length>=this.g)return a;if(this.m.length===this.g-1){const e=[...a].sort((f,c)=>f-c);return Array(e.length-1).fill(0).map((f,c)=>[e[c],e[c+1]])}if(0===this.g%2){const e=[];for(let f=0;f<a.length;f+=2)e.push([a[f],a[f+1]].sort((c,h)=>c-h));return 1===e.length?e[0]:e}const b=[...a].sort((e,f)=>e-f);return Array(b.length-
1).fill(0).map((e,f)=>[b[f],b[f+1]])}set values(a){a=[a].flat().map(b=>Array.isArray(b)?b:[b]).flat();if(a.length!==this.g)throw Error(this.g+" values expected");this.i=a.map(b=>Math.max(this.o,Math.min(this.T,+b)));this.u=[...this.i];y(this)}Y(a,b){this.J.addEventListener(a,b)}X(a,b){this.J.removeEventListener(a,b)}}var E=C,F=["Slider"],G=this||self;F[0]in G||"undefined"==typeof G.execScript||G.execScript("var "+F[0]);
for(var I;F.length&&(I=F.shift());)F.length||void 0===E?G[I]&&G[I]!==Object.prototype[I]?G=G[I]:G=G[I]={}:G[I]=E;C.prototype.off=C.prototype.X;C.prototype.on=C.prototype.Y;var D=self.EventTarget;}).call(this);
